...
local _M = {
	...
	<xml>
		...
		print(data.xml[1].test[1][1].id) 		 						=> one
		print(data.xml[1].test[1].text[1][2])  						=> This is text
		print(data.xml[1].test[3].some[1].deep[1].nest[1][1].x)		=> 100
	...
	--]]
	luaparse = function(buffer,length,attribute_callback,start_element_callback,end_element_callback)
		data = {}
	
		local parser = _L.XML_ParserCreate(nil)
		local last_content -- text within the tags
		local node = data -- current node in the tree
		...
		local function start_element(data,element,attribute)
			...
			while (attribute[idx] ~= nil and attribute[idx+1] ~= nil) do
				...
				new_node[1][key] = val
				if (attribute_callback) then
					attribute_callback(new_node,node,ele,key,val)
				end
				idx = idx + 2
			end
			...
			if (start_element_callback) then
				start_element_callback(new_node,parent_by_node[ new_node ],ele)
			end
		end
		...
		local function end_element(data,element)
			...
			if (end_element_callback) then
				end_element_callback(node,parent_by_node[ node ],ele)
			end
			node = parent_by_node[node] or data
		end
		...
		_L.XML_SetElementHandler(parser,start_element,end_element)
		_L.XML_SetCharacterDataHandler(parser,handle_data)
		
		if (_L.XML_Parse(parser, buffer, length, 1) == _L.XML_STATUS_ERROR) then
			print("expat:", ffi.string(_L.XML_ErrorString(_L.XML_GetErrorCode(parser))))
		end
		
		_L.XML_ParserFree(parser)
