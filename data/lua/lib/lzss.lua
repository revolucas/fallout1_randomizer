...
--------------------------------------------------------------------------------
local POS_BITS = 12
local LEN_BITS = 4--16 - POS_BITS
local POS_SIZE = 4096--bit.lshift(1,POS_BITS)
local LEN_SIZE = 12--bit.lshift(1,LEN_BITS)
local LEN_MIN = 3
		...
		for i = 0, 7 do
			if offset <= #input then
				...
				if pos and #str >= LEN_MIN then
					local tmp = bit.bor(( bit.lshift((pos - 1),LEN_BITS) ),(#str - LEN_MIN))
					buffer[#buffer + 1] = struct.pack('>I2', tmp)
				else
					flags = bit.bor(flags,( bit.lshift(1,i) ))
		...
		for i = 1, 8 do
			...
			if bit.band(flags,1) ~= 0 then
				...
				if offset + 1 <= #input then
					local tmp = struct.unpack('>I2', input, offset)
					offset = offset + 2
					local pos = bit.rshift(tmp,LEN_BITS) + 1
					local len = bit.band(tmp,(LEN_SIZE - 1)) + LEN_MIN
